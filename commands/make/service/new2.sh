...
# Exemple d'appel
REGEX='(?s)members\s*=\s*\[\s*\n(.*)\n\s*\]'

# Lancer le script Perl et capturer la sortie
output=$($(dirname "$0")/_maker_system/utils.pl "$CARGO_WORKSPACE_PATH" "$REGEX")

# Extraire les résultats dans des variables distinctes
IFS=$'|||' read -r capture first_line match_count <<< "$output"

# Affichage (ou traitement)
echo "Groupe capturé : $capture"
echo "Première ligne de match : $first_line"
echo "Nombre de lignes matchées : $match_count"
...
# last_port=$(grep -oE 'SERVICE_[A-Z_]+_PORT=[0-9]+' "$ENV_FILE_PATH" | awk -F= '{ print $2 }' | sort -nr | head -n1)
